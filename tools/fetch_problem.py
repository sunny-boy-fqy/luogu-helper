...
def get_with_retry(session, url, params=None):
    ...
    try:
        ...
        if r.text and "C3VK=" in r.text and not r.text.strip().startswith('{') and "lentille-context" not in r.text:
...
def find_contest_for_problem(session, prob_id, uid):
    if uid:
        try:
            r = get_with_retry(session, f"https://www.luogu.com.cn/record/list", params={"user": uid, "_contentOnly": "1"})
    ...
    try:
        r = get_with_retry(session, "https://www.luogu.com.cn/contest/list", params={"_contentOnly": "1"})
        if r is not None and r.status_code == 200:
            data = r.json()
            ...
            for contest in contests[:30]:
                ...
                if not cid: continue
                try:
                    cr = get_with_retry(session, f"https://www.luogu.com.cn/contest/{cid}", params={"_contentOnly": "1"})
                    if cr is not None and cr.status_code == 200:
                        cdata = cr.json()
                        problems = cdata.get('currentData', {}).get('contestProblems', [])
                        for p in problems:
                            if p.get('problem', {}).get('pid') == prob_id: return cid
...
def fetch_luogu_problem(prob_id, cookie=None, contest_id=None):
    ...
    session.headers.update({'User-Agent': 'Mozilla/5.0', 'Referer': 'https://www.luogu.com.cn/'})
    uid = None
    if cookie:
        for item in cookie.split(';'):
            if '=' in item:
                k, v = item.strip().split('=', 1); session.cookies.set(k, v, domain='www.luogu.com.cn')
                if k == '_uid': uid = v
    
    params = {'contestId': contest_id} if contest_id else {}
    res = get_with_retry(session, f"https://www.luogu.com.cn/problem/{prob_id}", params=params)
    
    if res is not None and (res.status_code in (401, 403) or (res.status_code == 200 and '无权' in res.text)) and not contest_id and prob_id.startswith(('T', 'U')):
        cid = find_contest_for_problem(session, prob_id, uid)
        if cid:
            res = get_with_retry(session, f"https://www.luogu.com.cn/problem/{prob_id}", params={'contestId': cid})

    if not res: return None
    
    match = re.search(r'<script id="lentille-context" type="application/json">(.*?)</script>', res.text)
    if match:
        try:
            data = json.loads(match.group(1))
            if data.get('template') == 'error': return None
            return data.get('data', {}).get('problem')
        except: pass
        
    try:
        data = res.json()
        if 'currentData' in data: return data['currentData'].get('problem')
        if 'data' in data and 'problem' in data['data']: return data['data']['problem']
    except: pass
    return None
...
def extract_code_blocks(content):
    ...
    for match in matches:
        code = match.strip()
        if code and ('#include' in code or 'int main' in code or 'void' in code or 'class' in code):
...
def fetch_luogu_solutions(prob_id, cookie=None, contest_id=None):
    ...
    params = {}
    if contest_id: params['contestId'] = contest_id
    res = get_with_retry(session, f"https://www.luogu.com.cn/problem/solution/{prob_id}", params=params)
    
    if res is not None and (res.status_code in (401, 403)) and not contest_id and prob_id.startswith(('T', 'U')):
        cid = find_contest_for_problem(session, prob_id, uid)
        if cid:
            res = get_with_retry(session, f"https://www.luogu.com.cn/problem/solution/{prob_id}", params={'contestId': cid})
    ...
    if match:
        ...
        try:
            data = res.json()
            solutions = data.get('currentData', {}).get('solutions', {}).get('result', [])
            if not solutions:
                solutions = data.get('data', {}).get('solutions', {}).get('result', [])
        except: return None
    ...
    for idx, sol in enumerate(solutions):
        ...
        if not code_blocks:
            ...
            if '#include' in content:
                ...
                started = False
                for line in lines:
                    if '#include' in line: started = True
                    if started: code_lines.append(line)
                if code_lines:
                    return '\n'.join(code_lines)
            continue
        ...
        if is_code_fragmented(code_blocks):
            print(f"  Warning: Solution {idx+1} appears to have fragmented code ({len(code_blocks)} blocks), trying next...")
            continue
    ...
    if first_solution_code:
        print(f"  Warning: All solutions have fragmented code. Using last block from solution 1")
...
def main():
    ...
    if args.action == "samples":
        ...
        if not p_data: print(f"Failed to fetch {prob_id}"); sys.exit(1)
        ...
        for i, s in enumerate(samples):
            ...
            with open(os.path.join(data_dir, f"data{i+1}.in"), "w") as f: f.write(inp.strip() + '\n')
            with open(os.path.join(data_dir, f"data{i+1}.out"), "w") as f: f.write(outp.strip() + '\n')
        with open(os.path.join(args.target_dir, "info"), "w") as f: f.write(f"{len(samples)} {tl}\n")
        ...
        if not content: content = p_data
        
        with open(os.path.join(args.target_dir, "problem.md"), "w", encoding='utf-8') as f:
            ...
            samples = p_data.get('samples', [])
            for i, s in enumerate(samples):
                inp, outp = (s[0], s[1]) if isinstance(s, list) else (s.get('input', ''), s.get('output', ''))
                f.write(f"### 输入样例 {i+1}\n\n```\n{inp.strip()}\n```\n\n")
                f.write(f"### 输出样例 {i+1}\n\n```\n{outp.strip()}\n```\n\n")
        ...
        if not code: print(f"Failed to fetch solution for {prob_id}"); sys.exit(1)
