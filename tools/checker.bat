...
if not exist "%PROB_DIR%\std.cpp" (
    ...
    REM Check if get_solve.exe exists in multiple locations
    set GET_SOLVE_EXE=%TOOLS_DIR%get_solve.exe
    if not exist "%GET_SOLVE_EXE%" (
        set GET_SOLVE_EXE=%TOOLS_DIR%..\tools\get_solve.exe
    )
    if not exist "%GET_SOLVE_EXE%" (
        set GET_SOLVE_EXE=%TOOLS_DIR%get_solve.bat
    )
    if not exist "%GET_SOLVE_EXE%" (
        set GET_SOLVE_EXE=%TOOLS_DIR%..\tools\get_solve.bat
    )
    
    if exist "%GET_SOLVE_EXE%" (
        echo Using: %GET_SOLVE_EXE%
        pushd "%PROB_DIR%"
        call "%GET_SOLVE_EXE%"
        popd
    ) else (
        echo Error: Could not find get_solve.exe or get_solve.bat
        echo Searched in:
        echo   - %TOOLS_DIR%get_solve.exe
        echo   - %TOOLS_DIR%..\tools\get_solve.exe
        echo   - %TOOLS_DIR%get_solve.bat
        echo   - %TOOLS_DIR%..\tools\get_solve.bat
        exit /b 1
    )
